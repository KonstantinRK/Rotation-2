...
def plot_results(results, random_counterpart=None, random_concepts=None, num_random_exp=100,
    ...
    plot_data = {}
    plot_concepts = []
    rnd_cn = random_counterpart
    output = [["target", "bottleneck", "concept", "counterpart", "tcav", "tcav_std", "rand", "rand_std", "pval", "significant"]]
    ...
    for concept in result_summary:
        ...
        if not is_random_concept(concept):
            ...
            for bottleneck in result_summary[concept]:
                ...
                output.append([results[0]['target_class'], bottleneck, concept, rnd_cn, np.mean(i_ups), np.std(i_ups),
                               np.mean(random_i_ups[bottleneck]),
                               np.std(random_i_ups[bottleneck]), p_val, p_val <= min_p_val])
